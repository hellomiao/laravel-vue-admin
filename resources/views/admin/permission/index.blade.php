...
@section('content')
    ...
    <div class="row page-title-row" id="dangqian" style="margin:5px;">
        <div class="col-md-6">
            @if($level==0)
                <span style="margin:3px;" id="cid" attr="{{$level}}" class="btn-flat text-info"> 顶级菜单</span>
            @else
                <span style="margin:3px;" id="cid" attr="{{$level}}" class="text-info"> {{$data->display_name}}
                        </span>
        ...
        <div class="col-md-6 text-right">
            @can('admin.permission.create')
                <a href="/admin/permission/{{$level}}/create" class="btn btn-success btn-md"><i class="fa fa-plus-circle"></i> 添加权限 </a>
            @endcan
        </div>
    </div>
